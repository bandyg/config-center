...
- [ ] **T5.3** `batch.html` DC 模式适配
  ...
  - 推送结果标注 BC 来源
  - 批量推送逻辑：按 BC 聚合 → 并行发送
- [x] **T5.4** `compare.html` DC 模式适配
  - 终端选择列表标注分支信息
- [x] **T5.5** 验证：DC 模式启动后浏览器功能走查（列表/详情/编辑/批量/对比/历史）
...
- [x] **T6.3** 前端压力测试
  ...
  - 确认浏览器不卡顿，分页正常
- [x] **T6.4** `servers.yaml` 配置 prod-ready
  - 预填 100 个 BC Server 的 URL
- [x] **T6.5** 更新 `build-windows.ps1`
  - `--add-data servers.yaml;.` 加入打包
- [x] **T6.6** 更新用户操作手册（`docs/用户操作手册.md`）
  - 增加 DC 模式部署章节

...
$TargetDir = Join-Path $ProjectRoot "app\static"
...
$files = @(
  ...
  @{ Url = "https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.18/addon/fold/foldgutter.css";    Path = "$CodemirrorDir\foldgutter.css" }
  @{ Url = "https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.18/addon/dialog/dialog.css";      Path = "$CodemirrorDir\dialog.css" }
  # diff-match-patch
...
Write-Host ""
if ($fail -eq 0) {
  Write-Host "Done: $ok downloaded, $fail failed" -ForegroundColor Green
} else {
